...
MISSING = object()
# silly way to check if argument is missing
...
if __name__ == "__main__":
    ...
    classifier_group.add_argument('--svm', action='store_true',
                                  help='study the impact of a growing testing set on the accuracy.')
    ...
    if args.load_features:
        ...
        file_list = pd.read_csv(args.images_list, header=None, names=['path', 'class', 'title'], engine='python')
        data = []
        data_class = []
        for index, i_path in file_list.iterrows():
            if os.path.exists(i_path['path']):
                page_image = Image.open(i_path['path'])
                data.append(extract_features_subresolution(page_image))
                data_class.append(i_path['class'])

        y = np.array(data_class)
